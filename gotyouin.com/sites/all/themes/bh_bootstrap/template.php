...
function bh_bootstrap_preprocess_html(&$variables) {
	...
	// Custom user handling stuff that Might come in use-ful. Will also be exported to JS in html.tpl
	if ( empty( $user ) || $user->uid === 0 ) {
		// anon user
		$user->is_logged_in = $user->is_admin = $user->is_customer = $user->is_regular_barber = $user->is_barber = $user->is_independent_barber = $user->is_owner = false;
	} else {
		$user->is_logged_in = true;
		$user->is_admin = in_array( 'admin', $user->roles ) || $user->name == 'admin' || $user->name == 'dbs_admin';
		$user->is_regular_barber = $user->is_barber = in_array( 'regular barber', $user->roles );
		$user->is_independent_barber = in_array( 'independent barber', $user->roles );
		$user->is_customer = in_array( 'customer', $user->roles );
		$user->is_owner = in_array( 'owner', $user->roles );
	}
	// $user->user_agent = $_SERVER['HTTP_USER_AGENT'];
	$user->is_ios = (preg_match("/iP(od|hone|ad)/i", $_SERVER["HTTP_USER_AGENT"]) ) ? true : false;
  ...
  global $theme;
  if ($theme == 'bh_bootstrap') {
    drupal_add_html_head_link(array('rel' => 'apple-touch-icon-precomposed', 'href' => DBS_STATIC . '/' .$path_to_icons . 'apple-touch-icon-144-precomposed.png', 'sizes' => '144x144'));
    drupal_add_html_head_link(array('rel' => 'apple-touch-icon-precomposed', 'href' => DBS_STATIC . '/' . $path_to_icons . 'apple-touch-icon-114-precomposed.png', 'sizes' => '114x114'));
    drupal_add_html_head_link(array('rel' => 'apple-touch-icon-precomposed', 'href' => DBS_STATIC . '/' . $path_to_icons . 'apple-touch-icon-72-precomposed.png', 'sizes' => '72x72'));
    drupal_add_html_head_link(array('rel' => 'apple-touch-icon-precomposed', 'href' => DBS_STATIC . '/' . $path_to_icons . 'apple-touch-icon-57-precomposed.png'));
  }
...
// print "<pre>"; print_r($user);
  ...
  if($user->roles[4] == 'owner')
  {
    ...
    $count=0;
    foreach($etids as $etid){ $count++; }
    if($count==0 &&  $_SERVER['REQUEST_URI'] != '/node/add/shop')
    {
      drupal_set_message(t('You were redirected because you need to create a shop!'), 'warning');
      drupal_goto('node/add/shop');
    }

  } 
  
  
  //if this person is a barber or an independent barber, nag them if no services set - JPC 2013-3-18
  //we only want to trigger this action if they are not already on a service add page
  //firstly, don't do anything at all if we are on the services add page
//  if ($_SERVER['REQUEST_URI'] != '/services-setup') {
//          if($user->roles[5] == 'independent barber' || $user->roles[6] == 'regular barber') {
//          $query = new EntityFieldQuery();
//
//          $query->entityCondition('entity_type', 'node')
...
//          
//          if(!$result) {
//               drupal_set_message(t('You were redirected because you need to add services to your profile - no one will be able to make an appointment with you until you do!'), 'warning');
//               drupal_goto('services-setup');
//          }
//      }
//  }
	...
	if (isset($variables['node'])) {
		$suggest = "page__node__edit__{$variables['node']->type}";
		$variables['theme_hook_suggestions'][] = $suggest;
	}

/*

if ((arg(0) == 'node') && (arg(2) == 'edit' && arg(2) == 'shop')) {
dump ( arg(4) );
...
// HB 2013-01-13    $bootstrap_path = '/sites/all/themes/bootstrap';
  ...
  // MS 2013-02-07 Add user roles classes to body tag
  ...
  // CSS
  drupal_add_css($bootstrap_path . '/bootstrap/css/bootstrap.css', array('media' => 'all'));
  drupal_add_css($bootstrap_path . '/bootstrap/css/bootstrap-responsive.css', array('media' => 'screen'));
  // JS
  drupal_add_js($bootstrap_path . '/bootstrap/js/bootstrap.min.js', array('scope' => 'footer'));
 ...
 */
function bh_bootstrap_preprocess_htmlmail(&$variables) {
...
/**
 * Use this to intercept date string fields from a view and format them
 * appropriately - allows us to display the date using the "plain" formatter
 * instead of the "Date and time" formatter that has the known timezone bug for
 * Drupal 7 Views. 2013-03-01
 * 
 * @param type $view
 */
function bh_bootstrap_views_pre_render(&$view) {
     foreach ($view->result as $value) {
          $text_date = str_replace( '-', '', strip_tags($value->field_field_date['0']['rendered']['#markup'] ) );
		$value->field_field_date['0']['rendered']['#markup'] = date('M d, Y - g:ia', strtotime($text_date));
     }
}

/**
* Implements hook_js_alter(): replacing jQuery with MUCH newer version
*/
function bh_bootstrap_js_alter( &$js ) { 
//file_put_contents( '/tmp/hal', print_r( $_SERVER, true ) );
	if ( strstr( $_SERVER['REQUEST_URI'], '/admin' ) ) return;
	if ( strstr( $_SERVER['REQUEST_URI'], 'import' ) ) return;
//	if ( strstr( $_SERVER['HTTP_REFERER'], 'edit?destination=admin' ) ) return;
// 2013-08-12, to fix js errors in admin for shop owners uploading files.
	if ( strstr( $_SERVER['QUERY_STRING'], 'destination=admin/content' ) ) return;
//	if ( preg_match( ',/node/\d+/edit,', $_SERVER['REQUEST_URI'] ) ) return;
	if (isset($js['misc/jquery.js'])) {
//				$jquery_path = '//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js';
				$jquery_path = '/misc/jquery-1.9.1.min.js';
//				$jquery_path = '//ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js';
...
/**
* Custom implementation to rewrite url's of static content, for cdn-like
* delivery.
*
...
Nice stuff but does not work on compressed js or css

function bh_bootstrap_file_url_alter(&$uri) {
	global $conf;
	$cdn = $conf['cdn_domain'];
	if ( empty( $cdn ) ) return;
	$uri = 'http://' . $cdn . '/' . $uri;
